...
library(tm)
library(SnowballC)
j_file5 <-"./talks_time.json"
talks_time<-fromJSON(file=j_file5, method="C")
str(talks_time)
talks_time<-fromJSON(file=j_file5, method="C")
str(talks_time)
talk_names<-names(talks_time$talks[[16]]$talk)
talk_names
###Convertobject type
talks_time_list<-lapply(talks_time$talks, function(x){unlist(x)})
str(talks_time_list)
parse_talk_all<-data.frame()
df_parse_talk_all<-do.call("rbind", c(parse_talk_all, talks_time_list))
...
png("./dendrogram_sparse_time_ave_k12.png", width = 1200, height=600)
plot(fit)
...
write.table(df_groups, "./tc_result_time_ave_k12.txt", row.names=FALSE, col.names=TRUE, sep="\t")
