...
library(tm)
library(SnowballC)
j_file3 <-"./talks_free.json"
talks_free<-fromJSON(file=j_file3, method="C")
str(talks_free)
talks_free<-fromJSON(file=j_file3, method="C")
str(talks_free)
talk_names<-names(talks_free$talks[[16]]$talk)
talk_names
###Convertobject type
talks_free_list<-lapply(talks_free$talks, function(x){unlist(x)})
str(talks_free_list)
...
plot(fit)
rect.hclust(fit, k=16)

###Save the dendrogram as PNG image file
png("./dendrogram_sparse_free_ave_k16.png", width = 1200, height=600)
plot(fit)
...
write.table(df_groups, "./tc_result_free_ave_k16.txt", row.names=FALSE, col.names=TRUE, sep="\t")
