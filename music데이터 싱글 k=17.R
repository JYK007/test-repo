...
library(tm)
library(SnowballC)
j_file4 <-"./talks_music.json"
talks_music<-fromJSON(file=j_file4, method="C")
str(talks_music)
talks_music<-fromJSON(file=j_file4, method="C")
str(talks_music)
talk_names<-names(talks_music$talks[[16]]$talk)
talk_names
###Convertobject type
talks_music_list<-lapply(talks_music$talks, function(x){unlist(x)})
str(talks_music_list)
parse_talk_all<-data.frame()
df_parse_talk_all<-do.call("rbind", c(parse_talk_all, talks_music_list))
...
plot(fit)
rect.hclust(fit, k=17)

###Save the dendrogram as PNG image file
png("./dendrogram_sparse_music_sin_k17.png", width = 1200, height=600)
plot(fit)
...
write.table(df_groups, "./tc_result_music_sin_k17.txt", row.names=FALSE, col.names=TRUE, sep="\t")
